...
class Settings:
    ...
    system_prompt: str = (
        ...
        "Most questions arrive with no image, and that is normal: answer them as "
        "conversation. An image is attached only when the user asked you to look, "
        "and when it is, it is what they are looking at right now; if several frames "
        "are provided they are recent views in time order (the last is current). "
        "If a question clearly needs to see something and no image is attached, say "
        "so briefly and tell them to say \"look\". "
